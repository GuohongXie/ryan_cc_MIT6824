#ifndef RYAN_DS_MAP_REDUCE_MR_WORKER_OLD_H_
#define RYAN_DS_MAP_REDUCE_MR_WORKER_OLD_H_
...
void* mapWorker(void* arg) {
  ...
  while (1) {
    ...
    string taskTmp = client.call<string>("AssignTask")
...
void* reduceWorker(void* arg) {
  ...
  while (1) {
    ...
    if (disabledReduceId == 1 || disabledReduceId == 3 ||
        disabledReduceId == 5) {
      disabledReduceId++;
      pthread_mutex_unlock(&map_mutex);
    ...
    client.call<bool>(
        "SetReduceStat",
        reduceTaskIdx);  //最终文件写入磁盘并发起RPCcall修改reduce状态
  }
}
...
#endif  // RYAN_DS_MAP_REDUCE_MR_WORKER_OLD_H_