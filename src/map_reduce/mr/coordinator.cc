...
class Coordinator {
 ...
 private:
  ...
  vector<int> reduce_index_;  //所有reduce任务的工作队列
  vector<std::string> running_map_work_;  //正在处理的map任务，分配出去就加到这个队列，用于判断超时处理重发
  int curr_map_index_;  //当前处理第几个map任务
  int curr_reduce_index_;  //当前处理第几个reduce任务
  vector<int> running_reduce_work_;  //正在处理的reduce任务，分配出去就加到这个队列，用于判断超时处理重发
};
