...
class Coordinator {
 ...
 private:
  ...
  std::unordered_map<std::string, int>
      finished_map_task_;  //存放所有完成的map任务对应的文件名
  std::unordered_map<int, int>
      finished_reduce_task_;  //存放所有完成的reduce任务对应的reduce编号
  std::vector<int> reduce_index_;  //所有reduce任务的工作队列
  std::vector<std::string>
      running_map_work_;  //正在处理的map任务，分配出去就加到这个队列，用于判断超时处理重发
  int curr_map_index_;    //当前处理第几个map任务
  int curr_reduce_index_;  //当前处理第几个reduce任务
  std::vector<int>
      running_reduce_work_;  //正在处理的reduce任务，分配出去就加到这个队列，用于判断超时处理重发
};
