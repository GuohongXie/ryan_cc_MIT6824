...
bool Master::IsAllMapAndReduceDone(){
    m_assign_lock.lock();
...
int main(int argc, char* argv[]){
    if(argc < 2){
        cout<<"missing parameter! The format is ./mrcoordinator_raw pg*.txt"<<endl;
        exit(-1);
    }
    // alarm(10);
    buttonrpc server;
    ...
    server.bind("map_num", &Master::map_num, &master);
    server.bind("reduce_num", &Master::reduce_num, &master);
    ...
    server.bind("IsAllMapAndReduceDone", &Master::IsAllMapAndReduceDone, &master);
    server.run();
    return 0;
}