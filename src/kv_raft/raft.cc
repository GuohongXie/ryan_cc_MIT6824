...
string Operation::getCmd() {
  string cmd = op + " " + key + " " + value + " " + to_string(clientId) + " " +
               to_string(requestId);
  return cmd;
}
...
class AppendEntriesArgs {
 public:
  // AppendEntriesArgs():m_term(-1), m_leaderId(-1), m_prevLogIndex(-1),
  // m_prevLogTerm(-1){
  //     //m_leaderCommit = 0;
  //     m_sendLogs.clear();
  // }
  ...
  friend Serializer& operator>>(Serializer& in, AppendEntriesArgs& d) {
    in >> d.m_term >> d.m_leaderId >> d.m_prevLogIndex >> d.m_prevLogTerm >>
        d.m_leaderCommit >> d.m_sendLogs;
    return in;
  }
  friend Serializer& operator<<(Serializer& out, AppendEntriesArgs d) {
    out << d.m_term << d.m_leaderId << d.m_prevLogIndex << d.m_prevLogTerm
        << d.m_leaderCommit << d.m_sendLogs;
    return out;
  }
};
...
class Raft {
 ...
 private:
  ...
  vector<ApplyMsg>
      m_msgs;  //在applyLogLoop中存msg的容易，每次存一条，处理完再存一条

  // bool installSnapShotFlag;
  // bool applyLogFlag;
  std::unordered_set<int>
      isExistIndex;  //用于在processEntriesLoop中标识append和install端口对应分配情况
};
...
void Raft::setBroadcastTime() {
  ...
  printf("before : %ld, %ld\n", m_lastBroadcastTime.tv_sec,
         m_lastBroadcastTime.tv_usec);
  ...
  while (!raft->dead) {
    ...
    while (1) {
      ...
      if (raft->m_state == CANDIDATE && during_time > timeOut) {
        ...
        while (raft->recvVotes <= raft->m_peers.size() / 2 &&
               raft->finishedVote != raft->m_peers.size()) {
...
void* Raft::callRequestVote(void* arg) {
  ...
  if (raft->cur_peerId == raft->m_peers.size() ||
      (raft->cur_peerId == raft->m_peers.size() - 1 &&
       raft->m_peerId == raft->cur_peerId)) {
    raft->cur_peerId = 0;
  }
  raft->m_lock.unlock();
...
RequestVoteReply Raft::requestVote(RequestVoteArgs args) {
  ...
  if (m_votedFor == -1 || m_votedFor == args.candidateId) {
    ...
    printf("[%d] vote to [%d] at %d, duration is %d\n", m_peerId,
           args.candidateId, m_curTerm, getMyduration(m_lastWakeTime));
    gettimeofday(&m_lastWakeTime, NULL);
  }
  saveRaftState();
  m_lock.unlock();
...
void* Raft::processEntriesLoop(void* arg) {
  ...
  while (!raft->dead) {
    ...
    // printf("sec : %ld, usec : %ld\n", raft->m_lastBroadcastTime.tv_sec,
    // raft->m_lastBroadcastTime.tv_usec);
...
void* Raft::sendInstallSnapShot(void* arg) {
  ...
  //     raft->m_peers[i].isInstallFlag ? 1 : 0);
  // }
  for (int i = 0; i < raft->m_peers.size(); i++) {
  ...
  // clientPeerId, raft->m_peers[clientPeerId].m_port.second);
  ...
  if (realMajorityMatchIndex > raft->m_commitIndex &&
      (realMajorityMatchIndex <= raft->m_lastIncludedIndex ||
       raft->m_logs[raft->idxToCompressLogPos(realMajorityMatchIndex)].m_term ==
           raft->m_curTerm)) {
...
InstallSnapSHotReply Raft::installSnapShot(InstallSnapShotArgs args) {
  ...
  if (args.lastIncludedIndex <= m_lastIncludedIndex) {
    ...
    if (args.lastIncludedIndex < lastIndex()) {
      if (m_logs[idxToCompressLogPos(lastIndex())].m_term !=
          args.lastIncludedTerm) {
        m_logs.clear();
      } else {
        vector<LogEntry> tmpLog(
            m_logs.begin() + idxToCompressLogPos(args.lastIncludedIndex) + 1,
            m_logs.end());
        m_logs = tmpLog;
      }
    } else {
...
void Raft::push_backLog(LogEntry log) { m_logs.push_back(log); }
...
void* Raft::sendAppendEntries(void* arg) {
  ...
  // raft->m_peers[clientPeerId].m_port.second);
  ...
  for (int i = raft->idxToCompressLogPos(args.m_prevLogIndex) + 1;
       i < raft->m_logs.size(); i++) {
    args.m_sendLogs += (raft->m_logs[i].m_command + "," +
                        to_string(raft->m_logs[i].m_term) + ";");
  }
  ...
  if (args.m_prevLogIndex == raft->m_lastIncludedIndex) {
    ...
    args.m_prevLogTerm =
        raft->m_logs[raft->idxToCompressLogPos(args.m_prevLogIndex)].m_term;
  }
  ...
  if (reply.m_success) {
    ...
    if (realMajorityMatchIndex > raft->m_commitIndex &&
        (realMajorityMatchIndex <= raft->m_lastIncludedIndex ||
         raft->m_logs[raft->idxToCompressLogPos(realMajorityMatchIndex)]
                 .m_term == raft->m_curTerm)) {
      raft->m_commitIndex = realMajorityMatchIndex;
    }
  }
  ...
  if (!reply.m_success) {
    if (reply.m_conflict_term != -1 && reply.m_conflict_term != -100) {
      ...
      for (int index = args.m_prevLogIndex; index > raft->m_lastIncludedIndex;
           index--) {
        if (raft->m_logs[raft->idxToCompressLogPos(index)].m_term ==
...
AppendEntriesReply Raft::appendEntries(AppendEntriesArgs args) {
  ...
  // is %d\n",
  //         m_peerId, args.m_leaderId, m_curTerm, args.m_term,
  //         getMyduration(m_lastWakeTime));
  gettimeofday(&m_lastWakeTime, NULL);
  ...
  if (args.m_prevLogIndex < m_lastIncludedIndex) {
    ...
    if (args.m_prevLogTerm !=
        m_lastIncludedTerm) {  //脑裂分区，少数派的snapShot不对，回归集群后需要更新自己的snapShot及log
      reply.m_conflict_index = 1;
      m_lock.unlock();
      return reply;
    }
    ...
    if (lastIndex() < args.m_prevLogIndex) {
      ...
      printf(
          ...
          "is %d\n",
          m_peerId, lastIndex(), args.m_leaderId, args.m_prevLogIndex,
    ...
    if (m_logs[idxToCompressLogPos(args.m_prevLogIndex)].m_term !=
        args.m_prevLogTerm) {
      printf(" [%d]'s prevLogterm : %d != [%d]'s prevLogTerm : %d\n", m_peerId,
             m_logs[idxToCompressLogPos(args.m_prevLogIndex)].m_term,
             args.m_leaderId, args.m_prevLogTerm);

      reply.m_conflict_term =
          m_logs[idxToCompressLogPos(args.m_prevLogIndex)].m_term;
      for (int index = m_lastIncludedIndex + 1; index <= args.m_prevLogIndex;
           index++) {
        if (m_logs[idxToCompressLogPos(index)].m_term ==
            reply.m_conflict_term) {
          reply.m_conflict_index =
...
StartRet Raft::start(Operation op) {
  ...
  if (state != LEADER) {
    ...
    // ret.m_curTerm, ret.isLeader == false ? 0 : 1);
    m_lock.unlock();
    return ret;
  }
...
void Raft::serialize() {
  string str;
  str += to_string(this->persister.cur_term) + ";" +
         to_string(this->persister.votedFor) + ";";
  str += to_string(this->persister.lastIncludedIndex) + ";" +
         to_string(this->persister.lastIncludedTerm) + ";";
...
void Raft::readRaftState() {
  ...
  printf(" [%d]'s term : %d, votefor : %d, logs.size() : %d\n", m_peerId,
         m_curTerm, m_votedFor, m_logs.size());
}
...
ApplyMsg Raft::getBackMsg() { return m_msgs.back(); }
...
void Raft::recvSnapShot(string snapShot, int lastIncludedIndex) {
  ...
  printf("[%d] : %d - %d = compressLen is %d\n", m_peerId, lastIncludedIndex,
         this->m_lastIncludedIndex, compressLen);
  this->m_lastIncludedTerm =
      m_logs[idxToCompressLogPos(lastIncludedIndex)].m_term;
  ...
  printf("[%d] persister.size is %d, lastIncludedIndex is %d\n", m_peerId,
         persister.logs.size(), m_lastIncludedIndex);
  m_lock.unlock();
}
...
void Raft::saveSnapShot() {
  ...
  int len =
      write(fd, persister.snapShot.c_str(), persister.snapShot.size() + 1);
  close(fd);
}
...
int Raft::lastIndex() { return m_lastIncludedIndex + m_logs.size(); }
