...
class GetArgs {
 public:
  ...
  friend Serializer& operator>>(Serializer& in, GetArgs& d) {
    in >> d.key >> d.client_id_ >> d.request_id_;
    return in;
  }
  friend Serializer& operator<<(Serializer& out, GetArgs d) {
    out << d.key << d.client_id_ << d.request_id_;
    return out;
  }
};
...
class PutAppendArgs {
 public:
  std::string key;
  std::string value;
  std::string op;  //区分操作类型put、append
  int client_id_;
  int request_id_;
  friend Serializer& operator>>(Serializer& in, PutAppendArgs& d) {
    in >> d.key >> d.value >> d.op >> d.client_id_ >> d.request_id_;
    return in;
  }
  friend Serializer& operator<<(Serializer& out, PutAppendArgs d) {
    out << d.key << d.value << d.op << d.client_id_ << d.request_id_;
    return out;
  }
};
...
std::string Clerk::Get(std::string key) {
  GetArgs args;
  args.key = key;
  args.client_id_ = client_id_;
  args.request_id_ = GetCurRequestId();
  int curr_leader = GetCurLeader();
...
std::vector<std::vector<int>> GetServerPort(int num) {
  std::vector<std::vector<int>> kv_server_port(num);  //TODO: 这里没错误吗
  for (int i = 0; i < num; i++) {
    for (int j = 0; j < 3; j++) {
