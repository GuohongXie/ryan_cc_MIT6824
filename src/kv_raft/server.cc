...
void KVServer::StartKvServer(vector<kvServerInfo>& kvInfo, int me,
                             int maxRaftState) {
  this->m_id = me;
...
void* KVServer::snapShotLoop(void* arg) {
  ...
  while (1) {
    ...
    // kv->m_lastAppliedIndex);
    if (kv->m_maxraftstate != -1 &&
        kv->m_raft.ExceedLogSize(
            kv->m_maxraftstate)) {  //设定了大小且超出大小则应用层进行快照
      kv->m_lock.lock();
...
bool KVServer::getRaftState() { return m_raft.getState().second; }

void KVServer::killRaft() { m_raft.kill(); }

void KVServer::activateRaft() { m_raft.activate(); }
