...
class KVServer {
 ...
 private:
  ...
  std::unordered_map<int, int> client_seq_map_;  //只记录特定客户端已提交的最大请求ID
...
void KVServer::StartKvServer(std::vector<KVServerInfo>& kvInfo, int me,
  ...
  std::thread(&KVServer::RPCServer, this).detach(); //创建多个用于监听客户端请求的RPCserver
  //pthread_t listen_tid1[port_.size()];  //创建多个用于监听客户端请求的RPCserver
...
std::string KVServer::GetSnapShot() {
  ...
  std::cout << "int cout snapShot is " << snapShot << std::endl;
...
void KVServer::RecoverySnapShot(std::string snapShot) {
  ...
  for (int i = 0; i < clientSeq.size(); i++) {
    ...
    client_seq_map_[std::atoi(tmp.c_str())] = std::atoi(value.c_str());
  }
  printf("-----------------databegin---------------------------\n");
...
int main() {
  ...
  while (1) {
    ...
    if (!kv_servers[i]->GetRaftState()) {
      kv_servers[i]->KillRaft();  //先让某个不是leader的raft宕机，不接受leader的appendEntriesRPC，让日志落后于leader的快照状态
      break;
    }
  }
  ::sleep(3);
  kv_servers[i]->ActivateRaft();  //重新激活对应的raft，在raft层发起installRPC请求，且向对应落后的kvServer安装从raft的leader处获得的快照
  //--------------------------------------Test---------------------------------------------
  while (1)
    ;
}
